    ...
    app.post('/inscription', async (req, res) => {
        ...
        try {
            ...
            connection.query(
                'INSERT INTO users (id, prenom, email, password, createdAt, updatedAt) VALUES (?, ?, ?, ?, NOW(), NOW())',
                [id, prenom, email, hashedPassword],
                (err, results) => {
                    if (err) {
                    ...
                    res.sendFile(path.join(__dirname, 'app', 'view', 'todo.html'));
                }
            );
        } catch (error) {
    ...
    app.get('/create', (req, res) => {
        res.sendFile(path.join(__dirname, 'app', 'view', 'create.html'));
    });
    ...
    app.post('/create', async (req, res) => {
        ...
        connection.query(
            'INSERT INTO todos (id, nom, description, status, userId, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, NOW(), NOW())',
            [id, nom, description, "publish", null],
            (err, results) => {
                if (err) {
                    console.error('Erreur lors de l\'ajout d\'un ToDo :', err);
                    res.status(500).send('Erreur lors de l\'ajout d\'un ToDo');
                    return;
                }
                res.sendFile(path.join(__dirname, 'app', 'view', 'todo.html'));
            }
